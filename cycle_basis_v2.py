...
def coarse_pipeline0(i, allowed, version=1, print_flag='False', norm_flag = 'no', feature_addition_flag =False):
    ...
    for j in range(i, i+50):
        if j >= n:
            continue
        tmp = pipeline0(j, allowed, version, print_flag=print_flag, norm_flag=norm_flag, feature_addition_flag=feature_addition_flag)
        result = result + [tmp]
    return result
...
def quick_compute_graphs_(allowed, norm_flag='yes', feature_addition_flag=False):
    ...
    result = Parallel(n_jobs=-1)(delayed(coarse_pipeline0)(i, allowed, norm_flag = norm_flag, feature_addition_flag=feature_addition_flag) for i in range(0,n,50))
...
def get_dgms(beta=np.array([0, 0, 0, 0, 1]), parallel='on', n_jobs=-1, hop_flag = 'n', basep = 0, rs=100, edge_fil='off'):
    ...
    if parallel == 'off':
        ...
        epd_dgms = [0] * n
        for i in range(n):
            (graphs[i], dgms[i], sub_dgms[i], super_dgms[i], epd_dgms[i]) = pipeline1(i, beta=beta, hop_flag=hop_flag, basep=basep, rs=rs, edge_fil=edge_fil)
        ...
        # print('get_dgms takes %s' % (time.time() - start))
...
def dgms_data(graph, beta, n_jobs, debug_flag, norm_flag='no', hop_flag='n', basep=-1, rs=100, edge_fil='off'):
    ...
    if (flag1 != 'success') or (flag2 != 'success'):
        ...
        databundle = get_dgms(beta=beta, hop_flag=hop_flag, basep=basep, rs= rs, edge_fil=edge_fil)  # haven't handle parallel case
...
def function_basis_test():
    ...
    def getG():
        ...
        return G
    g = getG()
    g_cc = function_basis(g, ['cc'], norm_flag='no'); print g_cc.node[1]
    g_cc_deg = function_basis(g_cc, ['fiedler']); print g_cc_deg.node[1]
    g_ = function_basis(g, ['cc', 'fiedler']); print g_.node[1]
    for v in g_.nodes():
...
def function_basis(g, allowed, norm_flag = 'no', recomputation_flag=False, transformation_flag=True):
    ...
    if np.array([('edge_' in s) for s in allowed]).all():
        ...
        for filtration in allowed:
            ...
            if filtration[-3:]=='ave':
                ...
                g_ricci_min = edge_filtration(g_ricci.copy(), function_type=filtration[:-6]+'min', sub_flag=True, average_flag=False)
                g_ricci_max = edge_filtration(g_ricci.copy(), function_type=filtration[:-6]+'max', sub_flag=False, average_flag=False)
                g_ricci = g_ricci_min
                for v in g_ricci.nodes():
                    assert filtration[:-6]+'min' in g_ricci.node[v].keys()
    ...
    # if transformation_flag==True:
        ...
        attribute_lists = g_ricci.nodes[anynode].keys()
...
def add_function_value(gi, fv_input='fv_test', edge_value='max'):
    ...
    # return (gi, fv_list)
@timefunction
def compute_graphs_(allowed, graph_isomorphisim, graph, norm_flag = 'no', feature_addition_flag=False, skip_dump_flag='yes'):
    ...
    if graph_isomorphisim == 'off':
        ...
        import random
        nx_graphs_ = [nx_graphs_[i] for i in sorted(random.sample(xrange(len(nx_graphs_)), 2000))]
        graphs_tmp = Parallel(n_jobs=n_jobs, batch_size='auto')(delayed(pipeline0)(i,allowed, version=2, norm_flag = norm_flag) for i in range(len(nx_graphs_)))
...
def kernelsvm(dist_matrix, Y, c, sigma, dist_flag='yes', print_flag='off'):
    ...
    print('svm kernel result: c: %s, sigma %s, train_acc: %s, test_acc: %s' %(c, sigma, accuracies['train_acc'], accuracies['test_acc']))
...
@timefunction
def searchclf(X, Y, i, test_size=0.1, nonlinear_flag='False', verbose=0, print_flag='off', laplacian_flag=False, lap_band=10):
...
def edge_baseline(edge_fil, graph, table):
        ...
        for ax in [0, 1]:
            ...
            (rf_, cv_score, t, svm_param, dgms_stat1, dgms_stat2, vct) = clf_pdvector(0, (sub_dgms, super_dgms, dgms), beta, Y, pd_flag, print_flag='off',
                                          nonlinear_flag=True, axis=axis, rf_flag=rf_flag,
...
def edge_btwn_features(upperbound=0.4, pro_flag=False, dummy_flag=False, n_jobs=20, print_flag=False):
    ...
    if dummy_flag:
        return np.zeros((n,1))
    from joblib import delayed, Parallel
    result = Parallel(n_jobs=n_jobs, batch_size='auto', verbose=5)(delayed(edge_btwn_i_feature)(i, upperbound, pro_flag=pro_flag, print_flag=print_flag) for i in range(n))
...
def edge_btwn_i_feature(i, upperbound=0.4, pro_flag = False, print_flag=False):
    ...
    for g in graphs_[i]:
        if g == None:
            continue
        edge_btwn_list += nx.edge_betweenness_centrality(g).values()
...
def edge_btwn_i_feature_(i, upperbound=0.4):
    ...
    for j in range(len(graphs_[i])):
        ...
        result = []
        for key in ['mean', 'min', 'max', 'std']:
            hisresult = nx.get_edge_attributes(g, key).values()
...
def basepoint_filtration():
    ...
    for b in sampleb:
        ...
        (graphs[s], dgms[s], sub_dgms[s], super_dgms[s]) = \
            dgms_data(graph, np.array(beta),n_jobs, debug_flag, hop_flag='y', basep=b, rs=seed)  # compute dgms for beta
        print('Dgm in different iteration:'),
    ...
    pd_vector_data = clf_pdvector(0, (dgms, nulldgm, dgms), beta, Y, pd_flag, print_flag='off', nonlinear_flag=True, dynamic_range_flag=dynamic_range_flag)
...
def coarse_graph_i(i, key='deg'):
    ...
    if key == 'deg':
        ...
        subgraphs = []
        for gi in gg:
            subgraphs += [gi.subgraph(c) for c in sorted(nx.connected_components(gi), key=len, reverse=True) if len(c)>3]
        # for g in subgraphs:
        #     print str(type(g))
        return subgraphs
...
def compute_coarsed_graphs_():
    ...
    for attr in ['deg', '1_0_deg_max', '1_0_deg_min', '1_0_deg_mean', '1_0_deg_std']:
        assert nx.get_node_attributes(coarse_graphs_[0][0], attr).keys() >= 0
...
def coarse_graph(g, print_flag=True):
    ...
    random.seed(42)
    n = len(nodelist)/2
    deg = dict(nx.degree(g)); n_edge = len(g.edges)
...
def evaluate_tda_kernel(tda_kernel, Y, best_result_so_far):
    ...
    if grid_search_re['score'] < best_result_so_far[0]-2:
    ...
    return (precision_format(100 * cv_score.mean(),1), precision_format(100 * cv_score.std(),1), grid_search_re, svm_time)

def mem(graph, baseline, rfclf, clf_search_offprint, evaluate_best_estimator, function_basis, get_diagram, sw_parallel, evaluate_tda_kernel, clf_pdvector,merge_features):
    ...
    return (baseline, rfclf, clf_search_offprint, evaluate_best_estimator, function_basis, get_diagram, sw_parallel, evaluate_tda_kernel, clf_pdvector, merge_features)
...
if __name__ == '__main__':
    ...
    # graph = 'protein_data'; rf_flag='no'; pa = 0; pb = 1
    print('pa and pb', pa, pb)
    print('graph is %s'%graph)
    # (baseline, rfclf, clf_search_offprint, evaluate_best_estimator, function_basis, get_diagram, sw_parallel,
     # evaluate_tda_kernel, clf_pdvector, merge_features) = mem(graph, baseline, rfclf, clf_search_offprint,
     #                                                          evaluate_best_estimator, function_basis, get_diagram,
     #                                                          sw_parallel, evaluate_tda_kernel, clf_pdvector,
     #                                                          merge_features)
    from joblib import Memory
    location = '/home/cai.507/Documents/DeepLearning/deep-persistence/' + graph + '/cachedir/'
    memory = Memory(location, verbose=1)
    # high_order = memory.cache(high_order)
    # clf_search_offprint = memory.cache(clf_search_offprint)
    ...
    Y = Y_origin
    n = len(Y_origin)
    # graphs_backup = compute_graphs_(['ricciCurvature'], graph_isomorphisim, graph, norm_flag='no', feature_addition_flag=False)
    ...
    # # ugly hack
    # # for i in range(n):
    # #     globals()[str(graphs_) + str(i)] = graphs_[i]

    def more_features(g, feature):
        g_ = function_basis(g, ['cc'], norm_flag='yes')
        print nx.get_node_attributes(g_, 'cc')

    # coarse graph
    if False:
        coarsed_graphs = coarse_graphs_()
        for gs in coarsed_graphs:
            print [len(g) for g in gs]
        coarsed_graphs_ = compute_coarsed_graphs_()
        g = coarsed_graphs_[0][0]
        anynode = list(g.nodes())[0]
        print g.node[anynode]
        graphs_ = copy.deepcopy(graphs_save)

    # node filtratillon
    norm_flag = 'yes'
    graphs_ = graphs_backup
    # graphs_ = compute_graphs_(allowed, graph_isomorphisim, graph, norm_flag=norm_flag, feature_addition_flag=False, load_flag='yes');
    landscape_data = []; itr = 0; svm_data_hist = {0: {}}
    betalist = set_betalist(allowed)
    # graphs__ = compute_graphs_(['ricciCurvature'], graph_isomorphisim, graph, norm_flag='yes', feature_addition_flag=True)
    # graphs_ = graphs__
    # graphs_ = compute_graphs_(allowed, graph_isomorphisim, graph, norm_flag=norm_flag)
    check_global_safety()
    for beta in betalist:
        ...
        from testground import aggregation_feature, searchclf, evaluate_clf, pairing_feature

        X = pairing_feature(dgms, n_bin=20)
        ...
        fake_dgms = fake_diagrams(graphs_, dgms, attribute=beta_name, seed=22, true_dgms=dgms)
        fake_dgms1 = fake_diagrams(graphs_, dgms, attribute=beta_name, seed=33, true_dgms=dgms)
        fake_dgms2 = fake_diagrams(graphs_, dgms, attribute=beta_name, seed=44, true_dgms=dgms)
        fake_dgms3 = fake_diagrams(graphs_, dgms, attribute=beta_name, seed=55, true_dgms=dgms)
        fake_dgms4 = fake_diagrams(graphs_, dgms, attribute=beta_name, seed=66, true_dgms=dgms)
        ...
        if True:
            ...
            for ax in [0, 1]:
                for epd_flag in [False]:
                    ...
                    for fake_dgms in [fake_dgms, fake_dgms1, fake_dgms2, fake_dgms3, fake_dgms4]:
                        pd_vector_data = clf_pdvector(best_vec_result, (fake_dgms, fake_dgms, fake_dgms, fake_dgms), beta, Y,
                                                      ...
                                                      dynamic_range_flag=dynamic_range_flag)  # use pd vector as baseline
                        table = add_row(table, pd_vector_data, beta_name, ax, filtration_type='node_vec')
                        add_row(table, pd_vector_data, name, ax, filtration_type='empty')
            ...
            for bandwidth in kernel_parameter(kernel_type)['bw']:
                for k in kernel_parameter(kernel_type)['K']:
                    for p in kernel_parameter(kernel_type)['p']:
                        ...
                        (fake_kernel, _) = (tda_kernel, t1) = sw_parallel(dgms2swdgm(fake_dgms), dgms2swdgm(fake_dgms), parallel_flag=True,
                                                           kernel_type=kernel_type, n_directions=10,
                                                           bandwidth=bandwidth, K=k, p=p)


                        try:
                            (tda_kernel, t1) = sw_parallel(dgms2swdgm(fake_dgms), dgms2swdgm(fake_dgms), parallel_flag=True,
                                                           kernel_type=kernel_type, n_directions=10,
                                                           bandwidth=bandwidth, K=k, p=p)

                            tda_kernel_data_ = evaluate_tda_kernel(tda_kernel, Y, best_result_so_far)
                            # (tda_kernel, t1) = sw_parallel(dgms2swdgm(dgms), dgms2swdgm(dgms), parallel_flag=True, kernel_type=kernel_type, n_directions=10, bandwidth=bandwidth, K=k, p=p)
                            # tda_kernel_data_ = evaluate_tda_kernel(tda_kernel, Y, best_result_so_far)

                        except:
                            print ('Problem occured for', kernel_type, bandwidth, k, p)
                            continue
