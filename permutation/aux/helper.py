...
from cycle_tools import make_direct, dgm2diag, remove_zero_col, print_dgm, flip_dgm
import numpy as np
import sys
...
def single_test(kernel, c, Y, r_seed, lbd = 1e-7, loss='hinge', debug='off', debug_flag='off', test_flag=False):
    ...
    print('.'),
    if test_flag == True:
        y_train, y_test, indices_train, indices_test = train_test_split(Y, range(n), test_size=1, random_state=r_seed)
    elif test_flag == False:
        y_train, y_test, indices_train, indices_test = train_test_split(Y, range(n), test_size=0.1, random_state=r_seed)
    n_train = len(y_train)
...
def sample_data(data, Y):
    idx = range(0, 500) + range(4000, 4500)
    d_slice = {key: value for key, value in data.items()
               if key in idx}
...
def functionongraph(graphs_, i, key='deg', edge_flag=False, short_cut_flag=False):
    ...
    for j in range(components):
        ...
        if edge_flag==False:
            ...
            tmp_min = [g.node[k][minkey] for k in g.nodes if ('type' in g.nodes[k].keys() and g.node[k]['type']=='edge_node')]
            tmp_max = [g.node[k][maxkey] for k in g.nodes if ('type' in g.nodes[k].keys() and g.node[k]['type']=='edge_node')]
            tmp = tmp_min + tmp_max
        lis += tmp
...
def get_nodeid(g, key, value, tol=1e-4):
    ...
    n_nodes = len(g)
    idx = list(g.nodes())[0]
    assert key in g.nodes[idx].keys()
    assert type(key) == str
...
def total_gradient(cache, idx, sigma, debug='off'):
    ...
    if debug == 'on':
        print(type(total)),
        print(total),
        print('the shape of total before sum is %s' % str(np.shape(total))),
        # total = np.zeros((1,5))
        # for i in idx:
        #     if debug == 'on':
        #         print(i),
        ...
        print(total)
        assert total[4]==0
    print('Computing total gradient takes %s .'%(time.time()-start)),
    return total
def check_trainloss(check_cache):
    ...
    print('Accuracy of old clf on traning set is %s .' % accuracy_score(np.sign(old_clf.predict(old_kernel_train)), y[train_idx])),
...
def total_loss(itr, cache, train_idx):
    ...
    print('The total loss on dataset of size %s is %s .'%(len(train_idx), total_loss)),
    # print(all_loss)
    import matplotlib.pyplot as plt
    ax = plt.subplot()
